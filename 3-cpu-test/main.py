  ...
  # Get all output details
    ...
    for i in range(count):
        if scores[i] >= threshold:
            result = {'bounding_box': boxes[i], 'class_id': classes[i],
                      'score': scores[i]}
...
def main():
    ...
    while(cap.isOpened()):
        ...
        while success:
            image = PIL.Image.fromarray(frame).convert('RGB').resize(
                (input_width, input_height), PIL.Image.ANTIALIAS)
